...
async function leerPDF() {
  ...
  if (!apiKey) { setStatus("❌ Ingresa tu API Key de Anthropic primero."); return; }
  if (!file)   { alert("Selecciona un archivo PDF primero."); return; }

  // Guardar key para próxima vez
...
Extrae TODAS las filas que tienen "Auto Serv.".
Para cada fila necesito:
- ID de bomba (número en la primera columna)
- Tipo de combustible de su sección: SUPER, REGULAR o DIESEL
- Valor de la columna "DIF USD LECT. DISP." (última columna, puede ser $0.00)

Responde ÚNICAMENTE con JSON, sin texto extra, sin bloques de código.
Formato: [{"bomba":1,"sabor":"S","monto":0.00},...]
sabor: "S"=SUPER "R"=REGULAR "D"=DIESEL. Monto sin símbolo $.`
            }
          ]
        }]
      })
    ...
    if (!Array.isArray(resultado) || !resultado.length) throw new Error("La IA no devolvió datos.");

    bombas = resultado
      .map(b => ({ bomba: parseInt(b.bomba, 10), sabor: b.sabor, monto: parseFloat(b.monto) }))
...
function generarFacturas(soloPositivas = false) {
  ...
  bombasAUsar.forEach(b => {
    let monto = Math.min(Math.round(b.monto * 100) / 100, restantePorSabor[b.sabor]);
    ...
    if (bacRestante > 0) {
      ...
      let tmp = bacBomba;
      while (tmp > 200) { facturas.push({ bomba: b.bomba, sabor: b.sabor, monto: 200.00, metodo: "B" }); tmp = Math.round((tmp-200)*100)/100; }
    ...
    if (monto <= 0) return;
    let tmp = monto;
    while (tmp > 200) { facturas.push({ bomba: b.bomba, sabor: b.sabor, monto: 200.00, metodo: "E" }); tmp = Math.round((tmp-200)*100)/100; }
